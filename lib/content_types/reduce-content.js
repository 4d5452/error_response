...
function getReduced(type, appContent) {
  ...
  let parse = (id, content) => {
    ...
    if(types.isPrimitive(content.type)) {// primitive type found: base case
      ...
      //test here for all required (allowable)
      types.isValid(content.type, ret);
      return ret;
    }
    // is type defined as a reference:
    if(types.isReference(content.type)) {// reference type found
    ...
    if(types.isObject(content.type)) {// object found
